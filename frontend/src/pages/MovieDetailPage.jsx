...
export default function MovieDetailPage() {
  ...
  const language  = Array.isArray(movie.language) ? movie.language.join(', ') : (movie.language || '')
  const isShowing = ['now_showing', 'nowShowing', 'now-showing', 'showing', 'active', 'upcoming'].includes(movie.status) || true
    ...
    <div className="min-h-screen pt-16">
      ...
      <div className="relative h-[50vh] md:h-[65vh] overflow-hidden">
        <div className="absolute inset-0 bg-[#030206]">
          {poster && (
            <img src={poster} alt={movie.title} className="w-full h-full object-cover opacity-40" />
          )}
        </div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-white/50">
                {movie.imdbRating > 0 && (
                  <span className="flex items-center gap-1 text-yellow-400 font-semibold">
                    ★ {movie.imdbRating}
                    {movie.reviewCount > 0 && <span className="text-white/30 text-xs font-normal">({movie.reviewCount} reviews)</span>}
                  </span>
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {movie.cast.map((member, i) => (
                    <motion.div key={i}
                      ...
                      <div>
                        ...
                        {member.role && <p className="text-xs text-white/40 leading-snug mt-0.5">{member.role}</p>}
                      </div>
                    </motion.div>
                  ))}
                </div>
            ...
            {trailer && (
              <section>
                ...
                <div className="relative aspect-video rounded-xl overflow-hidden bg-black">
                  <iframe src={trailer} className="w-full h-full" allowFullScreen title="Trailer" />
                </div>
              </section>
            )}
          ...
          <div>
            <motion.div initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }}
              ...
              {/* Rating summary in booking panel */}
              {movie.imdbRating > 0 && (
                <div className="flex items-center gap-2 mb-4 pb-4 border-b border-white/10">
                  <div className="flex gap-0.5">
                    {[1,2,3,4,5].map(s => (
                      <span key={s} style={{ color: movie.imdbRating/2 >= s ? '#fbbf24' : 'rgba(255,255,255,0.15)', fontSize:'14px' }}>★</span>
                    ))}
                  </div>
                  <span className="text-yellow-400 font-bold text-sm">{movie.imdbRating}</span>
                  {movie.reviewCount > 0 && <span className="text-white/30 text-xs">({movie.reviewCount})</span>}
                </div>
              )}
